...
export const core: typeof enCore = {
  ...
  statusBar: {
    ...
    inProgressText:
      "Пройдите каждый шаг ниже. Спортивная дирекция (СД) утверждает регистрацию после завершения всех шагов.",
    underReviewTitle: "Регистрация завершена",
    underReviewText:
      "Ваша регистрация завершена — все шаги отправлены. Спортивная дирекция (СД) проверит её, и после утверждения вы получите письмо с подтверждением.",
    confirmedTitle: "Утверждено — вы допущены к PATS 2026",
    confirmedTextWithDates: (dates) =>
      `Ваша регистрация утверждена СД, место подтверждено. Сроки: ${dates}.`,
    confirmedText: "Ваша регистрация утверждена СД, место подтверждено.",
  ...
  journey: {
    ...
    headers: {
      ...
      verification: {
        ...
        subtitle:
          "Спортивная дирекция (СД) утверждает регистрацию после завершения всех шагов.",
      },
      roster: {
    ...
    banners: {
      ...
      flightsSubmitted: "Данные о рейсе отправлены.",
      flightsSubmittedSub:
        "Ваша регистрация передана в Спортивную дирекцию (СД) на утверждение.",
      awaitingApproval: "Ожидает утверждения СД",
      awaitingApprovalSub:
        "Все шаги завершены. Спортивная дирекция рассмотрит и утвердит вашу регистрацию.",
      approvalLocked:
        "Завершите подтверждение, данные подразделения, состав команды и данные о рейсе, чтобы отправить регистрацию на утверждение.",
